			...
			<div class="portlet-body form">
				...
				<form action="" class="horizontal-form" method="get">
					...
					<div class="form-actions right">
						<button type="button" class="btn default" onclick="window.location='/Motor';">Limpiar</button>
...
<div class="portlet light">
	<div class="portlet-title">
		<div class="caption font-dark">
			...
			<span class="caption-subject bold uppercase">Motores</span>
		</div>
		<div class="tools"> </div>
	</div>
	<div class="portlet-body form">
					...
					<div class="row">
						<div class="col-md-12">
							<div class="dt-buttons">
								<a class="dt-button buttons-print btn dark btn-outline" tabindex="0" aria-controls="table_1" href="/Motor/agregar"><i class="fa fa-plus"></i> <span>Nuevo</span></a>
							</div>
						</div>
					</div>
					<div class="row">
						<div class="col-md-12">
							<div class="table-responsive">
								<table class="table table-striped table-bordered table-hover">
									<tr>
										<?php echo "<th style=\"width:50px;\">" . $paginator->sort('Motor.id', 'Cód.') . "</th>"; ?>
										<?php echo "<th>" . $paginator->sort('Motor.nombre', 'Motor') . "</th>"; ?>
										<?php echo "<th>" . $paginator->sort('TipoAdmision.nombre', 'Tipo Admisión') . "</th>"; ?>
										<?php echo "<th>" . $paginator->sort('TipoEmision.nombre', 'Tipo Emisión') . "</th>"; ?>
										<?php echo "<th>" . $paginator->sort('Motor.nombre', 'Modelo Motor') . "</th>"; ?>
										<?php echo "<th>" . $paginator->sort('Motor.nombre', 'Arreglo Motor') . "</th>"; ?>
										<th align="center">
											<?php
								...
								<?php
									foreach( $registros as $registro ) {
										echo "<tr>";
											echo "<td align=\"center\"><input type=\"hidden\" name=\"registro[{$registro['Motor']['id']}]\" value=\"{$registro['Motor']['e']}\">";
											echo "{$registro['Motor']['id']}</td>";
											echo "<td>{$registro['Motor']['nombre']}</td>";
											echo "<td>{$registro['TipoAdmision']['nombre']}</td>";
											echo "<td>{$registro['TipoEmision']['nombre']}</td>";
											echo "<td>{$registro['Motor']['nombre']} {$registro['TipoAdmision']['nombre']} {$registro['TipoEmision']['nombre']}</td>";
											echo "<td>{$registro['Motor']['nombre']} {$registro['TipoEmision']['nombre']}</td>";
											echo "<td align=\"center\" style=\"width: 60px;\">";
											echo '<div class="form-group" style="margin-bottom: 0;">';
											echo '<div class="mt-checkbox-inline">';
											echo '<label class="mt-checkbox mt-checkbox-outline">';
											if($registro['Motor']['e'] == '1') {
												echo "<input type=\"checkbox\" class=\"check-option\" name=\"estado[{$registro['Motor']['id']}]\" value=\"1\" checked=\"checked\" />";
											} else {
												echo "<input type=\"checkbox\" class=\"check-option\" name=\"estado[{$registro['Motor']['id']}]\" value=\"1\" />";
											}
											echo "<span></span>";
											echo "</label>";
											echo '</div>';
											...
											echo "<td align=\"center\" style=\"width: 80px\">";
												echo $this->Html->link("Editar", array('action' => 'editar', $registro['Motor']['id']), array('class' => 'btn btn-sm blue'));
											echo "</td>";
										echo "</tr>";
									}
								?>
