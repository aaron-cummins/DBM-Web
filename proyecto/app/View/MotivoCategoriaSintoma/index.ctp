			...
			<div class="portlet-body form">
				...
				<form action="" class="horizontal-form" method="get">
						...
						<div class="row">
							...
							<div class="col-md-4">
								<div class="form-group">
									<label>Motivo de llamado</label>
									<select class="form-control" name="motivo_id" id="motivo_id"> 
									<option value="">Todos</option>
									<?php foreach($motivos as $key => $value) { ?>
										<option value="<?php echo $key;?>" <?php echo @$motivo_id == $key ? 'selected="selected"' : ''; ?>><?php echo $value;?></option>
									<?php } ?>
								</select> 
								</div>
							</div>
							...
							<div class="col-md-4">
								<div class="form-group">
									<label>Categoría de síntoma</label>
									<select class="form-control" name="categoria_id" id="categoria_id"> 
									<option value="">Todos</option>
									<?php /*foreach($categorias as $key => $value) { ?>
										<option value="<?php echo $key;?>" <?php echo @$categoria_id == $key ? 'selected="selected"' : ''; ?>><?php echo $value;?></option>
									<?php }*/ ?>
								</select> 
								</div>
							</div>
							...
							<div class="col-md-4">
								<div class="form-group">
									...
									<?php /*foreach($sintomas as $key => $value) { ?>
										...
										<option value="<?php echo $value["Sintoma"]["id"];?>" <?php echo @$sintoma_id == $value["Sintoma"]["id"] ? 'selected="selected"' : ''; ?>><?php echo $value["Sintoma"]["codigo"];?> <?php echo $value["Sintoma"]["nombre"];?></option>
										<?php } else { ?>
										<option value="<?php echo $value["Sintoma"]["id"];?>" <?php echo @$sintoma_id == $value["Sintoma"]["id"] ? 'selected="selected"' : ''; ?>><?php echo $value["Sintoma"]["nombre"];?></option>
										<?php } ?>
									<?php }*/ ?>
								</select> 
								</div>
					...
					<div class="form-actions right">
						<button type="button" class="btn default" onclick="window.location='/MotivoCategoriaSintoma';">Limpiar</button>
...
<div class="portlet light">
	<div class="portlet-title">
		<div class="caption font-dark">
			...
			<span class="caption-subject bold uppercase">Matriz síntoma</span>
		</div>
		<div class="tools"> </div>
	</div>
	<div class="portlet-body form">
					...
					<div class="row">
						<div class="col-md-12">
							<div class="dt-buttons">
								<a class="dt-button buttons-print btn dark btn-outline" tabindex="0" aria-controls="table_1" href="/MotivoCategoriaSintoma/agregar"><i class="fa fa-plus"></i> <span>Nuevo</span></a>
							</div>
						</div>
					</div>
					<div class="row">
						<div class="col-md-12">
							<div class="table-responsive">
								<table class="table table-striped table-bordered table-hover">
									<tr>
										<?php echo "<th style=\"width:50px;\">" . $paginator->sort('MotivoCategoriaSintoma.id', 'Cód.') . "</th>"; ?>
										<?php echo "<th>" . $paginator->sort('Motivo.nombre', 'Motivo de llamado') . "</th>"; ?>
										<?php echo "<th>" . $paginator->sort('Categoria.nombre', 'Categoría de síntoma') . "</th>"; ?>
										<?php echo "<th>" . $paginator->sort('Sintoma.nombre', 'Síntoma') . "</th>"; ?>
										<th align="center">
										<?php
								...
								<?php
									foreach( $registros as $registro ) {
										...
										echo "<tr>";
											echo "<td align=\"center\"><input type=\"hidden\" name=\"registro[{$registro['MotivoCategoriaSintoma']['id']}]\" value=\"{$registro['MotivoCategoriaSintoma']['e']}\">";
											echo "{$registro['MotivoCategoriaSintoma']['id']}</td>";
											...
											echo '<div class="mt-checkbox-inline">';
											echo '<label class="mt-checkbox mt-checkbox-outline">';
											if($registro['MotivoCategoriaSintoma']['e'] == '1') {
												echo "<input type=\"checkbox\" class=\"check-option\" name=\"estado[{$registro['MotivoCategoriaSintoma']['id']}]\" value=\"1\" checked=\"checked\" />";
											} else {
												echo "<input type=\"checkbox\" class=\"check-option\" name=\"estado[{$registro['MotivoCategoriaSintoma']['id']}]\" value=\"1\" />";
											}
											echo "<span></span>";
											echo "</label>";
											echo '</div>';
											...
											echo "<td align=\"center\" style=\"width: 80px\">";
												echo $this->Html->link("Editar", array('action' => 'editar', $registro['MotivoCategoriaSintoma']['id']), array('class' => 'btn btn-sm blue'));
											echo "</td>";
										echo "</tr>";
									}
								?>
