...
?>   
    ...
    <div class="wrapper">
       <form method="GET" action="/Trabajo/revisar">
              ...
              <tbody>
				...
				<tr>
					...
					<td align="right" colspan="2">
						<input type="button" name="filtro_limpiar" value="Limpiar" class="greyishB" onclick="window.location='/Trabajo/revisar'; return false;" />
          ...
          <table cellpadding="0" cellspacing="0" width="100%" class="sTable dsp_resultados">
              <thead>
                  <tr style="font-weight: bold;">
					  ...
					  <?php if (intval($this->Session->read('faena_id')) == 0) { ?>
						<td  style="min-width: 50px;"><a href="/Trabajo/revisar?<?php 
						$o="";
					  ...
					  ?></a></td>
					  <?php } ?>
					  <td  style="min-width: 50px;"><a href="/Trabajo/revisar?<?php echo $url;  if (isset($this->request->query) && isset($this->request->query["order"]) && $this->request->query["order"] == "correlativo:asc") {
					  ...
					  ?></a></td>
					  <td  style="min-width: 50px;" nowrap><a href="/Trabajo/revisar?<?php
					  echo $url; 
                      ...
                      <td  style="min-width: 50px;"><a href="/Trabajo/revisar?<?php echo $url; 
                      ...
                      <td  style="min-width: 50px;" nowrap><a href="/Trabajo/revisar?<?php echo $url; 
					 ...
					 $o="";
					  ...
					  ?></a></td>
					<td  style="min-width: 50px;"><a href="/Trabajo/revisar?<?php echo $url; 
					if (isset($this->request->query) && isset($this->request->query["order"]) && $this->request->query["order"] == "fecha_sincronizacion:asc") {
						echo "&order=fecha_sincronizacion:desc";
					  } else {
						echo "&order=fecha_sincronizacion:asc";
					  }
					$o="";
					  if(@$this->request->query["order"] == "fecha_sincronizacion:asc"){
						$o="a";
					  }elseif(@$this->request->query["order"] == "fecha_sincronizacion:desc"){
						$o="d";
					  }
					  ?>" style="display: block;">Sincronización<?php 
					  if($o=="a"){
						echo '<img src="/images/icons/dark/arrowUp.png" style="padding-top: 3px;float: right;opacity: .7;" />';
						}elseif($o=="d"){
						echo '<img src="/images/icons/dark/arrowDown.png" style="padding-top: 3px;float: right;opacity: .7;" />';
			  ...
			  <?php 
				...
				foreach ($intervenciones as $intervencion) { 
					...
					//$i=strtotime($intervencion['Planificacion']['fecha'] . ' ' . $intervencion['Planificacion']['hora']);
					//$f=strtotime($intervencion['Planificacion']['fecha_termino'] . ' ' .$intervencion['Planificacion']['hora_termino']);
					//$d=($f-$i)/60;
					$fecha_termino = new DateTime($intervencion['Planificacion']['fecha_termino'] . ' ' .$intervencion['Planificacion']['hora_termino']);
			  ...
			  ?>
                  ...
                  <tr <?php echo $style;?> class="response_fix">
					<td><?php echo $i++;?></td>
					<?php if (intval($this->Session->read('faena_id')) == 0) { ?>
						<td nowrap><?php echo $intervencion['Faena']['nombre'];?></td>
					  <?php } ?>
					  <td><?php echo $intervencion['Planificacion']['correlativo'];?></td>
						<td><?php echo $intervencion['Planificacion']['folio'];?></td>
					...
					if (strtoupper($intervencion['Planificacion']['tipointervencion']) == 'MP') {
						...
						echo $util->getBacklogDescripcion($intervencion['Planificacion']['id']);
					} else {
					...
					try {
						if ($fecha_sincronizacion != '')
							echo $fecha_sincronizacion->format('d-m-Y h:i A');
					} catch (Exception $e) {}
					  ?></td>
					...
					?>
					  ...
					  <?php
						...
						//$url = $url == "rp" ? "bl" : $url;
					  ?>
					  <form action="/Trabajo/ex/<?php echo $intervencion['Planificacion']['id']; ?>" method="POST">
