			...
			<div class="portlet-body form">
				...
				<form action="" class="horizontal-form" method="get">
								...
								<div class="form-group">
									...
									<select class="form-control" name="motor_id" id="motor_id"> 
										...
										<?php foreach($motores as $key => $value) { ?>
											<option value="<?php echo $value["Motor"]["id"];?>"><?php echo $value["Motor"]["nombre"];?> <?php echo $value["TipoEmision"]["nombre"];?></option>
										<?php }?>
									</select>
								</div>
							</div>
					...
					<div class="form-actions right">
						<button type="button" class="btn default" onclick="window.location='/FaenaFlota';">Limpiar</button>
					...
					<div class="row">
						<div class="col-md-12">
							<div class="dt-buttons">
								<a class="dt-button buttons-print btn dark btn-outline" tabindex="0" aria-controls="table_1" href="/FaenaFlota/Agregar"><i class="fa fa-plus"></i> <span>Nuevo</span></a>
							</div>
						</div>
					</div>
					<div class="row">
							...
							<div class="table-responsive">
								...
								<?php
									foreach( $registros as $registro ) {
										//print_r($registro);
										echo "<tr>";
											echo "<td align=\"center\"><input type=\"hidden\" name=\"registro[{$registro['FaenaFlota']['id']}]\" value=\"{$registro['FaenaFlota']['e']}\">";
											echo "{$registro['FaenaFlota']['id']}</td>";
											echo "<td>{$registro['Faena']['nombre']}</td>";
											echo "<td>{$registro['Flota']['nombre']}</td>";
											echo "<td>{$registro['Motor']['nombre']} {$registro['Motor']['TipoEmision']['nombre']}</td>";
											echo "<td>{$registro['Flota']['aplicacion']}</td>";
											echo "<td>{$registro['Flota']['oem']}</td>";
											echo "<td>{$registro['FaenaFlota']['avance_teorico']}</td>";
											...
											if($registro['FaenaFlota']['e'] == '1') {
												echo "<input type=\"checkbox\" name=\"estado[{$registro['FaenaFlota']['id']}]\" value=\"1\" checked=\"checked\" />";
											} else {
												echo "<input type=\"checkbox\" name=\"estado[{$registro['FaenaFlota']['id']}]\" value=\"1\" />";
											}
											echo "<span></span>";
											echo "</label>";
											echo '</div>';
											...
											echo "<td align=\"center\" style=\"width: 80px\">";
												echo $this->Html->link("Editar", array('action' => 'Editar', $registro['FaenaFlota']['id']), array('class' => 'btn btn-sm blue'));
