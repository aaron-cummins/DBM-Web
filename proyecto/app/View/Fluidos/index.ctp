			...
			<div class="portlet-body form">
				...
				<form action="" class="horizontal-form" method="get">
					...
					<div class="form-actions right">
						<button type="button" class="btn default" onclick="window.location='/Fluidos';">Limpiar</button>
...
<div class="portlet light">
	<div class="portlet-title">
		<div class="caption font-dark">
			...
			<span class="caption-subject bold uppercase">Fluidos</span>
		</div>
		<div class="tools"> </div>
	</div>
	<div class="portlet-body form">
					...
					<div class="row">
						<div class="col-md-12">
							<div class="dt-buttons">
								<a class="dt-button buttons-print btn dark btn-outline" tabindex="0" aria-controls="table_1" href="/Fluidos/agregar"><i class="fa fa-plus"></i> <span>Nuevo</span></a>
							</div>
						</div>
					</div>
					<div class="row">
						<div class="col-md-12">
							<div class="table-responsive">
								<table class="table table-striped table-bordered table-hover">
									<tr>
										<?php echo "<th style=\"width:50px;\">" . $paginator->sort('Fluido.id', 'Cód.') . "</th>"; ?>
										<?php echo "<th>" . $paginator->sort('Fluido.nombre', 'Fluido') . "</th>"; ?>
										<?php echo "<th>" . $paginator->sort('FluidoUnidad.nombre', 'Unidad') . "</th>"; ?>
										<th>Tipo ingreso</th>
										<th align="center">
										<?php
								...
								<?php
									foreach( $registros as $registro ) {
										//print_r($faenas);
										echo "<tr>";
											echo "<td align=\"center\"><input type=\"hidden\" name=\"registro[{$registro['Fluido']['id']}]\" value=\"{$registro['Fluido']['e']}\">";
											echo "{$registro['Fluido']['id']}</td>";
											echo "<td>{$registro['Fluido']['nombre']}</td>";
											echo "<td>{$registro['FluidoUnidad']['nombre']}</td>";
											echo "<td>";
											$data = array();
											//echo $registro["Fluido"]["tipo_ingreso"];
											foreach($tipos_ingresos as $key => $value){
												if(in_array($key, explode(",", $registro["Fluido"]["tipo_ingreso"]))) {
													$data[] = $value;
												}
											}
											echo implode(", ", $data);
											...
											echo '<div class="mt-checkbox-inline">';
											echo '<label class="mt-checkbox mt-checkbox-outline">';
											if($registro['Fluido']['e'] == '1') {
												echo "<input type=\"checkbox\" class=\"check-option\" name=\"estado[{$registro['Fluido']['id']}]\" value=\"1\" checked=\"checked\" />";
											} else {
												echo "<input type=\"checkbox\" class=\"check-option\" name=\"estado[{$registro['Fluido']['id']}]\" value=\"1\" />";
											}
											echo "<span></span>";
											echo "</label>";
											echo '</div>';
											...
											echo "<td align=\"center\" style=\"width: 80px\">";
												echo $this->Html->link("Editar", array('action' => 'editar', $registro['Fluido']['id']), array('class' => 'btn btn-sm blue'));
