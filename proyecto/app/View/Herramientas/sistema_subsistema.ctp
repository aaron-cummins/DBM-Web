...
<div class="wrapper">
	...
	?>
		<fieldset>
		<div class="title"><img src="/images/icons/dark/list.png" alt="" class="titleIcon"><h6>Archivo <?php echo $titulo; ?></h6></div>
...
<?php 
	if (count($this->request->data) && count($data) > 0) {
		echo $this->Form->create('Herramientas', array('action' => 'SistemaSubsistemaProcess'));
?>
	 <div class="widget">
		<fieldset>
		...
		<table cellpadding="0" cellspacing="0" width="100%" class="sTable">
			...
			<tr>
				...
				<td>Posición</td>
                <td>
					<select id="aPosicionSistema" class="flCargas">
						<option value="">Mostrar Todo</option>
						<?php
							foreach ($posicionessubsistemas as $var) { 
								echo "<option value=\"{$var["Posiciones_Subsistema"]["id"]}\">{$var["Posiciones_Subsistema"]["nombre"]}</option>"."\n"; 
							}
						?>
					</select>
				</td>
          ...
          <table cellpadding="0" cellspacing="0" width="100%" class="sTable tbResultados">
              ...
              <tbody>
				...
				foreach ($data as $fila) {
					...
					echo "<td class=\"posicionsubsistema\">{$fila["Posicion"]}</td>";
					if ($fila["Accion"] == '') {
				...
				foreach ($resultado as $fila) {
					...
					echo "<td class=\"posicionsubsistema\">".@$fila["Posicion"]["nombre"]."</td>";
					if (@$fila["MotorSistemaSubsistemaPosicion"]["e"] != '1') {
						echo "<td class=\"e\">Inactivo</td>";
						echo "<td><input type=\"checkbox\" name=\"deshacer[]\" class=\"matriz_opciones\" value=\"".@$fila["MotorSistemaSubsistemaPosicion"]["id"]."\" title=\"Marcar para deshacer eliminación\" /></td>";
					} else {
						echo "<td class=\"e\">Activo</td>";
						echo "<td><input type=\"checkbox\" name=\"quitar[]\" class=\"matriz_opciones\" value=\"".@$fila["MotorSistemaSubsistemaPosicion"]["id"]."\" title=\"Marcar para eliminar\" /></td>";
					}
					echo "</tr>";
				}
				?>
