...
<div class="wrapper">
	...
	<?php 
		if (count($this->request->data)) {
			echo $this->Form->create('Herramientas', array('action' => 'ElementoDiagnosticoProcess'));		
	?>
	 <div class="widget">
		<fieldset>
		...
		<table cellpadding="0" cellspacing="0" width="100%" class="sTable">
			...
			<tr>
				...
				<td>Diagnóstico</td>
				<td>
					<select id="aDiagnostico" class="flCargas">
						<option value="">Mostrar Todo</option>
						<?php
							foreach ($diagnosticos as $var) { 
								echo "<option value=\"{$var["Diagnostico"]["id"]}\">{$var["Diagnostico"]["nombre"]}</option>"."\n"; 
							}
						?>
					</select>
				</td>
	...
	<div class="widget">
          ...
          <table cellpadding="0" cellspacing="0" width="100%" class="sTable tbResultados">
              ...
              <tbody>
				...
				foreach ($resultados as $fila) {
					...
					echo "<td class=\"idelemento\">-</td>";
					echo "<td class=\"elemento\">".@$fila["Elemento"]["nombre"]."</td>";
					echo "<td class=\"diagnostico\">".@$fila["Diagnostico"]["nombre"]."</td>";
					if (@$fila["MotorSistemaSubsistemaElementoDiagnostico"]["e"] != '1') {
						echo "<td class=\"e\">Inactivo</td>";
						echo "<td><input type=\"checkbox\" name=\"deshacer[]\" class=\"matriz_opciones\" value=\"".@$fila["MotorSistemaSubsistemaElementoDiagnostico"]["id"]."\" title=\"Marcar para deshacer eliminación\" /></td>";
					} else {
						echo "<td class=\"e\">Activo</td>";
						echo "<td><input type=\"checkbox\" name=\"quitar[]\" class=\"matriz_opciones\" value=\"".@$fila["MotorSistemaSubsistemaElementoDiagnostico"]["id"]."\" title=\"Marcar para eliminar\" /></td>";
					}
					echo "</tr>";
				}*/
				?>
		...
		<?php
		//echo $this->Form->hidden('fm', array('value' => $fm));
