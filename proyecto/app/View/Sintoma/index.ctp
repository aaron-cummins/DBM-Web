			...
			<div class="portlet-body form">
				...
				<form action="" class="horizontal-form" method="get">
					...
					<div class="form-actions right">
						<button type="button" class="btn default" onclick="window.location='/Sintoma';">Limpiar</button>
					...
					<div class="row">
						<div class="col-md-12">
							<div class="dt-buttons">
								<a class="dt-button buttons-print btn dark btn-outline" tabindex="0" aria-controls="table_1" href="/Sintoma/agregar"><i class="fa fa-plus"></i> <span>Nuevo</span></a>
							</div>
						</div>
					</div>
					<div class="row">
						<div class="col-md-12">
							<div class="table-responsive">
								<table class="table table-striped table-bordered table-hover">
									<tr>
										<?php echo "<th style=\"width:50px;\">" . $paginator->sort('Sintoma.id', 'Cód.') . "</th>"; ?>
										<?php echo "<th>" . $paginator->sort('Sintoma.nombre', 'Síntoma') . "</th>"; ?>
										<?php echo "<th>" . $paginator->sort('Sintoma.codigo', 'Código de Falla') . "</th>"; ?>
										<th align="center">
										<?php
								...
								<?php
									foreach( $registros as $registro ) {
										//print_r($Sintomas);
										echo "<tr>";
											echo "<td align=\"center\"><input type=\"hidden\" name=\"registro[{$registro['Sintoma']['id']}]\" value=\"{$registro['Sintoma']['e']}\">";
											echo "{$registro['Sintoma']['id']}</td>";
											echo "<td>{$registro['Sintoma']['nombre']}</td>";
											if (is_numeric($registro['Sintoma']['codigo']) && intval($registro['Sintoma']['codigo']) > 0 ){
												echo "<td>{$registro['Sintoma']['codigo']}</td>";
											}else{
												echo "<td>&nbsp;</td>";
											}
											...
											echo '<div class="mt-checkbox-inline">';
											echo '<label class="mt-checkbox mt-checkbox-outline">';
											if($registro['Sintoma']['e'] == '1') {
												echo "<input type=\"checkbox\" class=\"check-option\" name=\"estado[{$registro['Sintoma']['id']}]\" value=\"1\" checked=\"checked\" />";
											} else {
												echo "<input type=\"checkbox\" class=\"check-option\" name=\"estado[{$registro['Sintoma']['id']}]\" value=\"1\" />";
											}
											echo "<span></span>";
											echo "</label>";
											echo '</div>';
											...
											echo "<td align=\"center\" style=\"width: 80px\">";
												echo $this->Html->link("Editar", array('action' => 'editar', $registro['Sintoma']['id']), array('class' => 'btn btn-sm blue'));
											echo "</td>";
										echo "</tr>";
									}
								?>
