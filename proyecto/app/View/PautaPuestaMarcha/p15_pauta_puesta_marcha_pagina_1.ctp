<div class="row">
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		<div class="portlet light">
			<div class="portlet-title">
				<div class="caption font-dark">
					<i class="icon-settings font-dark"></i>
					<span class="caption-subject bold uppercase">Pauta Puesta en Marcha</span>
				</div>
				<div class="tools">
					<button type="button" class="btn btn-sm green" onclick="window.location='/PautaPuestaMarcha/Ver/1/<?php echo $id;?>';">Página 1</button>
					<button type="button" class="btn btn-sm default" onclick="window.location='/PautaPuestaMarcha/Ver/2/<?php echo $id;?>';">Página 2</button>
					<button type="button" class="btn btn-sm default" onclick="window.location='/PautaPuestaMarcha/Ver/3/<?php echo $id;?>';">Página 3</button>
					<button type="button" class="btn btn-sm default" onclick="window.location='/PautaPuestaMarcha/Ver/4/<?php echo $id;?>';">Página 4</button>
					<button type="button" class="btn btn-sm default" onclick="window.location='/PautaPuestaMarcha/Ver/5/<?php echo $id;?>';">Página 5</button>
					<button type="button" class="btn btn-sm default" onclick="window.location='/PautaPuestaMarcha/Ver/6/<?php echo $id;?>';">Página 6</button>
					<button type="button" class="btn btn-sm default" onclick="window.location='/PautaPuestaMarcha/Ver/7/<?php echo $id;?>';">Página 7</button>
					<button type="button" class="btn btn-sm default" onclick="window.location='/PautaPuestaMarcha/Ver/8/<?php echo $id;?>';">Página 8</button>
					<button type="button" class="btn btn-sm default" onclick="window.location='/PautaPuestaMarcha/Ver/9/<?php echo $id;?>';">Página 9</button>
					<button type="button" class="btn btn-sm default" onclick="window.location='/PautaPuestaMarcha/Ver/10/<?php echo $id;?>';">Página 10</button>
				</div>
			</div>
			<div class="portlet-body form">
				<form action="" class="horizontal-form" method="post" id="form-pauta-mantencion">
					
					<div id="table_1_wrapper" class="dataTables_wrapper">
						<div class="form-body">						
							<div class="row">
								<div class="col-md-12">
									<div class="table-responsive">										
										
<table width="100%" border="1" align="center" cellpadding="5" cellspacing="0" id="tblPautaMantencion" class="table table-striped table-bordered table-hover">
  <tr>
    <td colspan="2"></td>
    <td colspan="6" align="center" bgcolor="#333333" style="color: #fff; padding: 10px;"><strong>DATOS</strong></td>
  </tr>
  <tr>
    <td colspan="2" valign="middle">Fecha de Inspección</td>
    <td colspan="6"><input type="date" name="Ppm_01_fecha_inspeccion" id="Ppm_01_fecha_inspeccion" /></td>
  </tr>
  <tr>
    <td colspan="2" valign="middle">Técnicos participantes</td>
    <td colspan="6"><?php 
	    				foreach($usuarios as $usuario){
		    				echo "{$usuario["Usuario"]["nombres"]} {$usuario["Usuario"]["apellidos"]}<br />";
	    				}
	?></td>
  </tr>
  <tr>
    <td colspan="2" valign="middle">Asesor Técnico</td>
    <td colspan="6">
		<select name="Ppm_01_asesor_tecnico" id="Ppm_01_asesor_tecnico">
			  <option value="">N/A</option>
			  <?php
	    		foreach($asesores as $asesor){					//echo $asesor;
		    		echo "<option value=\"{$asesor["Asesor"]["id"]}\">{$asesor["Asesor"]["nombre"]}</option>";		
		    	}
		    	?>
		</select>
	</td>
  </tr>
  <tr>
    <td colspan="2" valign="middle" bgcolor="#333333" style="color: #fff; padding: 10px;"><strong>INFORMACIÓN DEL EQUIPO</strong></td>
    <td colspan="6" align="center" bgcolor="#333333" style="color: #fff; padding: 10px;"><strong>DATOS</strong></td>
  </tr>
  <tr >
    <td colspan="2" valign="middle">Fecha puesta en servicio (dd/mm/aa)</td>
    <td colspan="6" ><input type="date" name="" id="Ppm_01_fecha_puesta_servicio"></td>
  </tr>
  <tr>
    <td colspan="2" valign="middle">Modelo del motor</td>
    <td colspan="6" ><input type="text" name="Ppm_01_equipo_modelo_motor" id="Ppm_01_equipo_modelo_motor"></td>
  </tr>
  <tr >
    <td colspan="2" valign="middle">ESN (Nº de Serie Motor)</td>
    <td colspan="6" ><input type="text" name="Ppm_01_equipo_esn" id="Ppm_01_equipo_esn" readonly></td>
  </tr>
  <tr>
    <td colspan="2" valign="middle">CPL</td>
    <td colspan="6" ><input type="text" name="Ppm_01_equipo_cpl" id="Ppm_01_equipo_cpl"></td>
  </tr>
  <tr >
    <td colspan="2" valign="middle" >Fabricante    del Equipo</td>
    <td colspan="6" ><input type="text" name="Ppm_01_equipo_fabricante" id="Ppm_01_equipo_fabricante"></td>
  </tr>
  <tr>
    <td colspan="2" valign="middle" >Modelo del Equipo</td>
    <td colspan="6" ><input type="text" name="Ppm_01_equipo_modelo" id="Ppm_01_equipo_modelo"></td>
  </tr>
  <tr >
    <td colspan="2" valign="middle" >Serie del Equipo</td>
    <td colspan="6" ><input type="text" name="Ppm_01_equipo_serie" id="Ppm_01_equipo_serie"></td>
  </tr>
  <tr>
    <td colspan="2" valign="middle" >Número Interno del equipo</td>
    <td colspan="6" ><input type="text" name="Ppm_01_equipo_numero_interno" id="Ppm_01_equipo_numero_interno"></td>
  </tr>
    <tr>
    <td colspan="2" valign="middle" >Tipo de equipo (Aplicación)</td>
    <td colspan="6" ><input type="text" name="Ppm_01_equipo_aplicacion" id="Ppm_01_equipo_aplicacion" readonly></td>
  </tr>
  <tr >
    <td colspan="2" valign="middle" >Horometro Quantum al momento de la instalación</td>
    <td colspan="5" ><input type="text" name="Ppm_01_equipo_horometro_quantum_instalacion" id="Ppm_01_equipo_horometro_quantum_instalacion"></td>
    <td width="38">&nbsp;&nbsp;Hr</td>
  </tr>
   <tr >
    <td colspan="2" valign="middle" >Horometro motor al momento de la instalación</td>
    <td colspan="5" ><input type="text" name="Ppm_01_equipo_horometro_instalacion" id="Ppm_01_equipo_horometro_instalacion"></td>
    <td width="38">&nbsp;&nbsp;Hr</td>
  </tr>
  <tr>
    <td colspan="2" valign="middle" >Estatus    del Equipo (especificar nuevo o usado)</td>
    <td colspan="6" >
		<select name="Ppm_01_status_equipo" id="Ppm_01_status_equipo">
			<option selected="selected" value="" disabled="disabled">Estatus</option>
			<option value="N">Nuevo</option>
			<option value="U">Usado</option>
	    </select>
    </td>
  </tr>
  <tr>
    <td colspan="2" valign="middle" bgcolor="#333333"  style="color: #fff; padding: 10px;"><strong>INFORMACIÓN DEL CLIENTE</strong></td>
    <td colspan="6" align="center" bgcolor="#333333" style="color: #fff; padding: 10px;"><strong>DATOS</strong></td>
  </tr>
  <tr >
    <td colspan="2" valign="middle" >Tipo de equipo (Aplicación)</td>
    <td colspan="6" ><input type="text" name="Ppm_01_cliente_aplicacion" id="Ppm_01_cliente_aplicacion"></td>
  </tr>
  <tr>
    <td colspan="2" valign="middle" >Horometro Quantum al momento de la instalación</td>
    <td colspan="6" ><input type="text" name="Ppm_01_cliente_horometro_quantum_instalacion" id="Ppm_01_cliente_horometro_quantum_instalacion"></td>
  </tr>
  <tr >
    <td colspan="2" valign="middle" >Horometro motor al momento de la instalación</td>
    <td colspan="6" ><input type="text" name="Ppm_01_cliente_horometro_instalacion" id="Ppm_01_cliente_horometro_instalacion"></td>
  </tr>
  <tr>
    <td colspan="2" valign="middle" bgcolor="#333333" style="color: #fff; padding: 10px;"><strong>INFORMACIÓN DEL DISTRIBUIDOR</strong></td>
    <td colspan="6" align="center" bgcolor="#333333" style="color: #fff; padding: 10px;"><strong>DATOS</strong></td>
  </tr>
  <tr >
    <td colspan="2" valign="middle" >Tipo de equipo (Aplicación)</td>
    <td colspan="6" ><input type="text" name="Ppm_01_distribuidor_aplicacion" id="Ppm_01_distribuidor_aplicacion"></td>
  </tr>
  <tr>
    <td colspan="2" valign="middle" >Horometro Quantum al momento de la instalación</td>
    <td colspan="6" ><input type="text" name="Ppm_01_distribuidor_horometro_quantum_instalacion" id="Ppm_01_distribuidor_horometro_quantum_instalacion"></td>
  </tr>
  <tr >
    <td colspan="2" valign="middle" >Horometro motor al momento de la instalación</td>
    <td colspan="6" ><input type="text" name="Ppm_01_distribuidor_horometro_instalacion" id="Ppm_01_distribuidor_horometro_instalacion"></td>
  </tr>
</table>

									</div>
								</div>
							</div>
						</div>
					</div>
					
					<div class="form-actions right">
						<input type="hidden" name="pagina" value="01" />
						<button type="button" class="btn default" onclick="window.location='/Trabajo/Detalle2/<?php echo $id;?>';">Volver a Intervención</button>
						<?php if (isset($estado) && $estado != "4") { ?>
						<button type="button" class="btn blue btn-pauta-mantencion">
							<i class="fa fa-save"></i> Guardar</button>
						<?php } ?>
					</div>
					
				</form>
			</div>
		</div>
	</div>
</div>

<div id="confirma_guardado" class="modal fade" tabindex="-1" data-backdrop="static" data-keyboard="false" data-attention-animation="false">
	<div class="modal-body">
        <p> ¿Realmente desea guardar los cambios ingresados? </p>
    </div>
    <div class="modal-footer">
        <button type="button" data-dismiss="modal" class="btn btn-outline red">Cancelar</button>
        <button type="button" class="btn green btn-submit-modal">Guardar</button>
    </div>
</div>

<div id="modal_mensaje_error" class="modal fade" tabindex="-1" data-backdrop="static" data-keyboard="false" data-attention-animation="false">
	<div class="modal-body">
        <p class="mensaje_modal"></p>
    </div>
    <div class="modal-footer">
        <button type="button" data-dismiss="modal" class="btn btn-outline red">Aceptar</button>
    </div>
</div>