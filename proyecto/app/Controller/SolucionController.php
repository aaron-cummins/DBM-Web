<?php
	...
	class SolucionController extends AppController {
		...
		public function agregar(){
			...
			if ($this->request->is('post')){
				try {
					if ($this->Solucion->save($this->request->data)){
		...
		public function index() {
			...
			if ($this->request->is('get')){
				...
				if(isset($this->request->query['estado']) && is_numeric($this->request->query['estado'])) {
					$conditions["Solucion.e"] = $this->request->query['estado'];
				}
			...
			$this->paginate = array('limit' => $limit, 'order' => 'Solucion.nombre', 'recursive' => 1, 'conditions' => $conditions);
			$this->set('registros', $this->paginate('Solucion'));
			$this->set('limit', $limit);
			$this->set('titulo', 'Solucion');
			$this->set('breadcrumb', 'Administración');
			
			// Filtros
			//$Solucions = $this->Solucion->find('list', array('order'=>'nombre'));
			//$fecha = date('Y-m-d', time() - 30*24*60*60);
			//$fecha_termino = date('Y-m-d');
		
			//$this->set('Solucions', $Solucions);
		...
		public function editar() {
			...
			if( $this->Solucion->exists() ){
				...
				if( $this->request->is( 'post' ) || $this->request->is( 'put' ) ){
					...
					$this->request->data = $this->Solucion->read();
