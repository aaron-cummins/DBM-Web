<?php
	...
	class FluidoIngresoController extends AppController {
		public function index() {
				...
				try {
					foreach($this->request->data['registro'] as $key => $value) {
						if(isset($this->request->data['btn-guardar'])) {
							if ($value == '0') {
								if (isset($this->request->data['estado']) && isset($this->request->data['estado'][$key]) && $this->request->data['estado'][$key] == '1') {
									...
									$data["id"] = $key;
									$data["e"] = "1";
									$this->FluidoTipoIngreso->save($data);
								}
							}
							if ($value == '1') {
						...
						if(isset($this->request->data['btn-eliminar'])) {
							if (isset($this->request->data['estado']) && isset($this->request->data['estado'][$key]) && $this->request->data['estado'][$key] == '1') {
								$this->FluidoTipoIngreso->delete($key);
							}
						}
					}
			...
			if ($this->request->is('get')){
				...
				if(isset($this->request->query['estado']) && is_numeric($this->request->query['estado'])) {
					$conditions["FluidoTipoIngreso.e"] = $this->request->query['estado'];
				}
			...
			$this->paginate = array('limit' => $limit, 'order' => 'FluidoTipoIngreso.nombre', 'recursive' => -1, 'conditions' => $conditions);
			$this->set('registros', $this->paginate('FluidoTipoIngreso'));
			$this->set('limit', $limit);
			$this->set('titulo', 'Tipo ingreso de fluidos');
		...
		public function agregar(){
			...
			if ($this->request->is('post')){
				try {
					if ($this->FluidoTipoIngreso->save($this->request->data)){
		...
		public function editar() {
			...
			//set the user id
			$this->FluidoTipoIngreso->id = $id;
			 
			//check if a user with this id really exists
			if( $this->FluidoTipoIngreso->exists() ){
				...
				if( $this->request->is( 'post' ) || $this->request->is( 'put' ) ){
					...
					$this->request->data = $this->FluidoTipoIngreso->read();
