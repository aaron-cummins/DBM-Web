<?php
	...
	class FaenaController extends AppController {
		...
		public function agregar(){
			...
			$this->loadModel('FaenaZona');
			$this->loadModel('FaenaZonaNumerica');
			...
			$zonas = $this->FaenaZona->find('list');
			$this->set(compact('zonas'));
			$zonas_numericas = $this->FaenaZonaNumerica->find('list');
			$this->set(compact('zonas_numericas'));
			$this->set('titulo', 'Faena');
		...
		public function index() {
			...
			if ($this->request->is('get')){
				...
				if(isset($this->request->query['estado']) && is_numeric($this->request->query['estado'])) {
					$conditions["Faena.e"] = $this->request->query['estado'];
				}
			...
			$this->paginate = array('limit' => $limit, 'order' => 'Faena.nombre', 'recursive' => 1, 'conditions' => $conditions);
			$this->set('registros', $this->paginate('Faena'));
