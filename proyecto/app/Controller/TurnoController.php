		...
		public function index() {
				...
				try {
					foreach($this->request->data['registro'] as $key => $value) {
						...
						if(isset($this->request->data['btn-eliminar'])) {
							if (isset($this->request->data['estado']) && isset($this->request->data['estado'][$key]) && $this->request->data['estado'][$key] == '1') {
								$this->Turno->delete($key);
								$this->Session->setFlash('Registro elimnado correctamente.','guardar_exito');
							}
						}
					}
				} catch(Exception $e) {
			...
			if ($this->request->is('get')){
				...
				if(isset($this->request->query['estado']) && is_numeric($this->request->query['estado'])) {
					$conditions["Turno.e"] = $this->request->query['estado'];
				}
			...
			$this->paginate = array('limit' => $limit, 'order' => 'Turno.nombre', 'recursive' => -1, 'conditions' => $conditions);
			$this->set('registros', $this->paginate('Turno'));
			$this->set('limit', $limit);
			$this->set('titulo', 'Turno');
		...
		public function editar() {
			...
			if( $this->Turno->exists() ){
				...
				if( $this->request->is( 'post' ) || $this->request->is( 'put' ) ){
					...
					$this->request->data = $this->Turno->read();
