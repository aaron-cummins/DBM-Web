		...
		public function index() {
			...
			if ($this->request->is('get')){
				...
				if(isset($this->request->query['estado']) && is_numeric($this->request->query['estado'])) {
					$conditions["Periodo.e"] = $this->request->query['estado'];
				}
			...
			$this->paginate = array('limit' => $limit, 'order' => 'Periodo.nombre', 'recursive' => -1, 'conditions' => $conditions);
			$this->set('registros', $this->paginate('Periodo'));
			$this->set('limit', $limit);
			$this->set('titulo', 'Período');
