...
class PrebacklogMotivoAceiteController extends AppController {

    function index() {
        $this->set('titulo', 'Motivo aceite');
                ...
                foreach ($this->request->data['registro'] as $key => $value) {
                    if ($value == '0') {
                        if (isset($this->request->data['estado']) && isset($this->request->data['estado'][$key]) && $this->request->data['estado'][$key] == '1') {
                            ...
                            $this->Prebacklog_motivoAceite->save($data);
        ...
        $this->paginate = array(
            'fields' => array('Prebacklog_motivoAceite.*'),
            'order' => 'Prebacklog_motivoAceite.id',
            'limit' => $limit,
            'recursive' => 1
        );
        $registros = $this->paginate('Prebacklog_motivoAceite');

        $this->set(compact('registros'));
        $this->set(compact('limit'));
    }

    function motivo_aceite($id = '') {
        $this->set('titulo', 'Motivo Aceite');
        $this->layout = 'metronic_principal';
        $this->loadModel('Prebacklog_motivoAceite');
        ...
        if ($id != "" && is_numeric($id)) {
            $PrebacklogAlerta = $this->Prebacklog_motivoAceite->find('first', array("conditions" => array("id" => $id), "fields" => "Prebacklog_motivoAceite.*", 'recursive' => -1));
