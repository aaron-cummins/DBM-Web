<?php
	...
	class CargoController extends AppController {
		public function index() {
			...
			if ($this->request->is('get')){
				...
				if(isset($this->request->query['estado']) && is_numeric($this->request->query['estado'])) {
					$conditions["NivelUsuario.e"] = $this->request->query['estado'];
				}
			...
			$this->paginate = array('limit' => $limit, 'order' => 'NivelUsuario.nombre', 'recursive' => -1, 'conditions' => $conditions);
			$this->set('registros', $this->paginate('NivelUsuario'));
			$this->set('limit', $limit);
			$this->set('titulo', 'Cargo');
		...
		public function agregar(){
			...
			if ($this->request->is('post')){
				try {
					if ($this->NivelUsuario->save($this->request->data)){
		...
		public function editar() {
			...
			//set the user id
			$this->NivelUsuario->id = $id;
			 
			//check if a user with this id really exists
			if( $this->NivelUsuario->exists() ){
				...
				if( $this->request->is( 'post' ) || $this->request->is( 'put' ) ){
					...
					$this->request->data = $this->NivelUsuario->read();
