<?php
	...
	class TipoSalidaController extends AppController {
		public function agregar(){
			...
			if ($this->request->is('post')){
				try {
					if ($this->TipoSalida->save($this->request->data)){
		...
		public function index() {
			...
			if ($this->request->is('get')){
				...
				if(isset($this->request->query['estado']) && is_numeric($this->request->query['estado'])) {
					$conditions["TipoSalida.e"] = $this->request->query['estado'];
				}
			...
			$this->paginate = array('limit' => $limit, 'order' => 'TipoSalida.nombre', 'recursive' => -1, 'conditions' => $conditions);
			$this->set('registros', $this->paginate('TipoSalida'));
		...
		public function editar() {
			...
			$this->TipoSalida->id = $id;
			if( $this->TipoSalida->exists() ){
				if( $this->request->is( 'post' ) || $this->request->is( 'put' ) ){
					...
					$this->request->data = $this->TipoSalida->read();
