<?php
	...
	class EstadosEquipoController extends AppController {
		...
		public function agregar(){
			...
			if ($this->request->is('post')){
				try {
					if ($this->EstadoEquipo->save($this->request->data)){
			...
			$this->set('titulo', 'Estados Equipo');
		...
		public function index() {
			...
			if ($this->request->is('get')){
				...
				if(isset($this->request->query['estado']) && is_numeric($this->request->query['estado'])) {
					$conditions["EstadoEquipo.e"] = $this->request->query['estado'];
				}
			...
			$this->paginate = array('limit' => $limit, 'order' => 'EstadoEquipo.nombre', 'recursive' => 1, 'conditions' => $conditions);
			$this->set('registros', $this->paginate('EstadoEquipo'));
		...
		public function editar() {
			...
			//set the user id
			$this->EstadoEquipo->id = $id;
			 
			//check if a user with this id really exists
			if( $this->EstadoEquipo->exists() ){
				...
				if( $this->request->is( 'post' ) || $this->request->is( 'put' ) ){
					...
					$this->request->data = $this->EstadoEquipo->read();
