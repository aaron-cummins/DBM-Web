<?php
	...
	class EstadosMotorInstalacionController extends AppController {
		...
		public function agregar(){
			...
			if ($this->request->is('post')){
				try {
					if ($this->EstadoMotorInstalacion->save($this->request->data)){
		...
		public function index() {
					...
					foreach($this->request->data['registro'] as $key => $value) {
						if ($value == '0') {
							if (isset($this->request->data['estado']) && isset($this->request->data['estado'][$key]) && $this->request->data['estado'][$key] == '1') {
								...
								$data["id"] = $key;
								$data["e"] = "1";
								$this->EstadoMotorInstalacion->save( $data );
							}
						}
						if ($value == '1') {
			...
			if ($this->request->is('get')){
				...
				if(isset($this->request->query['estado']) && is_numeric($this->request->query['estado'])) {
					$conditions["EstadoMotorInstalacion.e"] = $this->request->query['estado'];
				}
			...
			$this->paginate = array('limit' => $limit, 'order' => 'EstadoMotorInstalacion.nombre', 'recursive' => 1, 'conditions' => $conditions);
			$this->set('registros', $this->paginate('EstadoMotorInstalacion'));
		...
		public function editar() {
			...
			//set the user id
			$this->EstadoMotorInstalacion->id = $id;
			 
			//check if a user with this id really exists
			if( $this->EstadoMotorInstalacion->exists() ){
				...
				if( $this->request->is( 'post' ) || $this->request->is( 'put' ) ){
					...
					$this->request->data = $this->EstadoMotorInstalacion->read();
