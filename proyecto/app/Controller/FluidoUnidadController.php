<?php
	...
	class FluidoUnidadController extends AppController {
		public function index() {
			...
			if ($this->request->is('get')){
				...
				if(isset($this->request->query['estado']) && is_numeric($this->request->query['estado'])) {
					$conditions["FluidoUnidad.e"] = $this->request->query['estado'];
				}
			...
			$this->paginate = array('limit' => $limit, 'order' => 'FluidoUnidad.nombre', 'recursive' => -1, 'conditions' => $conditions);
			$this->set('registros', $this->paginate('FluidoUnidad'));
			$this->set('limit', $limit);
			$this->set('titulo', 'Unidad del fluido');
		...
		public function agregar(){
			...
			if ($this->request->is('post')){
				try {
					if ($this->FluidoUnidad->save($this->request->data)){
		...
		public function editar() {
			...
			//set the user id
			$this->FluidoUnidad->id = $id;
			 
			//check if a user with this id really exists
			if( $this->FluidoUnidad->exists() ){
				...
				if( $this->request->is( 'post' ) || $this->request->is( 'put' ) ){
					...
					$this->request->data = $this->FluidoUnidad->read();
